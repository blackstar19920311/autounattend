...
/**
 * Windows 11 autounattend.xml gener├ítor.
 * ├ërv├ęnyes unattend XML-t ├íll├şt el┼Ĺ a megadott konfigur├íci├│ alapj├ín.
 */
...
/**
 * Sz├ętbont egy Base64 k├│dolt szkriptet kis darabokra (chunkokra), ├ęs a RunSynchronous
 * szekci├│hoz adja ┼Ĺket cmd.exe echo seg├şts├ęg├ęvel. ├Źgy megker├╝lhet┼Ĺ a 259 karakteres XML limit.
 */
function addBase64ScriptToSyncCmds(runSyncCmds, orderRef, scriptContent, tempB64Path, destPs1Path) {
  ...
  const chunkSize = 200; // Biztons├ígosan a 259 karakteres Path limit alatt
...
/**
 * Sz├ętbont egy Base64 k├│dolt szkriptet FirstLogonCommands form├ítumban.
 * A szkriptet Base64-be k├│dolja, darabokra bontja, majd certutil-lal dek├│dolja.
 * NEM futtatja a szkriptet k├Âzvetlen├╝l ÔÇô a RunAll.ps1 "Karmester" fogja megh├şvni.
 * @returns {string} A dek├│dolt .ps1 f├íjl el├ęr├ęsi ├║tja (a Karmester gy┼▒jti ├Âssze).
 */
...
${scriptContent}
    ...
    Add-Content -Path $FullLog -Value "" -Encoding utf8
    Add-Content -Path $FullLog -Value "[SIKERES] A szegmens hib├ítlanul lefutott." -Encoding utf8
} catch {
    Add-Content -Path $ShortLog -Value "[$((Get-Date).ToString('HH:mm:ss'))] HIB├üS: $ScriptName" -Encoding utf8
    Add-Content -Path $FullLog -Value "" -Encoding utf8
  ...
  for (let i = 0; i < base64.length; i += chunkSize) {
    ...
    commands.push({
      ...
      description: `${description} (adatok ├şr├ísa ${i + 1}. r├ęsz)`
    });
  }

  commands.push({
    command: `certutil.exe -decode -f ${tempB64Path} ${destPs1Path}`,
    description: `${description} (dek├│dol├ís)`
  });

  // A futtat├íst NEM itt v├ęgezz├╝k ÔÇô a RunAll.ps1 Karmester fogja sorban megh├şvni
  return destPs1Path;
}
...
function buildSpecialize(config, componentAttrs) {
  const lines = [];
  lines.push('  <!-- specialize ÔÇô Sz├ím├şt├│g├ępn├ęv ├ęs hardver-megker├╝l├ęs -->');
  ...
  if (useRandom) {
    // Ideiglenes n├ęv ÔÇö a v├ęgleges nevet a PowerShell szkript ├íll├ştja be
    lines.push('      <ComputerName>TEMPNAME</ComputerName>');
  } else {
    // Fix n├ęv, nincs random ut├│tag
    lines.push(`      <ComputerName>${escapeXml(prefix)}</ComputerName>`);
  }
  lines.push('    </component>');

  // Deployment komponens: random sz├ím├şt├│g├ępn├ęv + bypass
  let order = 1;
  const runSyncCmds = [];

  // Random sz├ím├şt├│g├ępn├ęv gener├íl├ís telep├şt├ęskor
  // Minta: PREFIX-AB12 (2 nagybet┼▒ + 2 sz├ímjegy, fix hossz├║s├íg├║)
  if (useRandom) {
    // PowerShell szkript: gener├íl egy nevet ├ęs egy h├ítt├ęrfolyamatban (loop) folyamatosan
    // be├şrja a registry-be, mert a Windows specialize f├ízis fel├╝l├şrhatja.
    const psScript = `
...
`;
    ...
    runSyncCmds.push('        <!-- Egyedi sz├ím├şt├│g├ępn├ęv gener├íl├ísa telep├şt├ęskor (h├ítt├ęrfolyamattal) -->');
  ...
  // --- Alv├ís letilt├ísa ├ęs Maxim├ílis teljes├ştm├ęny energias├ęma (Specialize f├ízis) ---
  if (config.disableSleep) {
    runSyncCmds.push('');
    runSyncCmds.push('        <!-- Alv├ís letilt├ísa ├ęs Maxim├ílis teljes├ştm├ęny energias├ęma -->');
    const powerScript = `
...
`;
  ...
  // --- T├ílca ikonok (HKLM H├ízirendek a Specialize f├ízisban) ---
  if (config.hideTaskbarIcons) {
    runSyncCmds.push('');
    runSyncCmds.push('        <!-- T├ílca r├Âgz├şt├ęsek tilt├ísa (HKLM H├ízirend) -->');
...
`;
  ...
  // --- Windows 11 T├ílcaikonok ├ęs Vizu├ílis be├íll├şt├ísok (Logon Scheduled Task) ---
  let vtScript = [];
  ...
  vtScript.push('}'); // Z├írjuk le a "csak egyszer fut├│" blokkot!
...
`;
    runSyncCmds.push('');
    runSyncCmds.push('        <!-- Windows 11: VisualTweaksTask (Egyszer fut├│ Logon Feladat a vizu├ílis elemekhez) -->');
    const orderRef = { val: order };
    addBase64ScriptToSyncCmds(runSyncCmds, orderRef, vtTaskXml, 'C:\\Windows\\Temp\\vt_task.b64', 'C:\\Windows\\Temp\\vt_task.ps1');
    order = orderRef.val;
  }

  // --- Bloatware elt├ívol├şt├ísa a Windows lemezk├ępb┼Ĺl (Specialize f├ízis) ---
  ...
  if (bloatScript.length > 2) {
    runSyncCmds.push('');
    runSyncCmds.push('        <!-- Bloatware elt├ívol├şt├ísa a Windows lemezk├ępb┼Ĺl -->');
    const orderRef = { val: order };
...
function buildOobeSystem(config, componentAttrs, inputLocale, uiLanguage) {
  const lines = [];
  lines.push('  <!-- oobeSystem ÔÇô Felhaszn├íl├│i fi├│k, OOBE ├ęs testreszab├ís -->');
  ...
  if (config.autoLogin && config.username && config.username.trim() !== '') {
    lines.push('');
    lines.push('      <!-- Automatikus bejelentkez├ęs -->');
  ...
  if (commands.length > 0) {
    lines.push('');
    lines.push('      <!-- Els┼Ĺ bejelentkez├ęskor fut├│ parancsok -->');
...
// ---------------------------------------------------------------------------
// FirstLogonCommands ├Âssze├íll├şt├ísa
// ---------------------------------------------------------------------------
function buildFirstLogonCommands(config, uiLanguage) {
  const commands = [];
  const scriptPaths = []; // Karmester: ├Âsszegy┼▒jti az ├Âsszes .ps1 el├ęr├ęsi utat

  // --- 1 perces glob├ílis v├írakoz├ís ---
  // A felhaszn├íl├│ k├ęr├ęs├ęre az ├Âsszes FirstLogon szkript el┼Ĺtt v├írunk 1 percet (60 m├ísodperc).
  ...
  if (needsWait) {
    commands.push({
      ...
      description: '1 perc v├írakoz├ís a h├íl├│zat fel├íll├ís├íra a k├╝ls┼Ĺ telep├şt┼Ĺk el┼Ĺtt',
    });
  }

  if (config.username) {
    // --- Jelsz├│ lej├írat├ínak letilt├ísa ---
    ...
    commands.push({
      ...
      description: 'Jelsz├│ lej├írat├ínak letilt├ísa',
    });
  }

  // --- H├íl├│zat megker├╝l├ęse ---
  if (config.bypassNetwork) {
    commands.push({
      command: 'cmd /c reg add "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\OOBE" /v BypassNRO /t REG_DWORD /d 1 /f',
      description: 'H├íl├│zati k├Âvetelm├ęny megker├╝l├ęse',
    });
  }

  // --- Wi-Fi be├íll├şt├ísok ---
  if (config.wifi) {
...
`;
      scriptPaths.push(addBase64ScriptToFirstLogon(
        ...
        `Automatikus csatlakoz├ís a(z) ${ssid} Wi-Fi h├íl├│zathoz`
      ));
    } else if (config.wifi.mode === 'manual') {
      commands.push({
        command: `cmd /c start ms-availablenetworks:`,
        description: 'Wi-Fi h├íl├│zatok list├íj├ínak megjelen├şt├ęse els┼Ĺ bejelentkez├ęskor',
      });
    }
  }
  ...
  // Start men├╝ takar├şt├ís kiker├╝lt innen, most a specialize f├ízisban van (LayoutModification.json)

  if (config.disableTelemetry) {
    commands.push({
      command: 'cmd /c reg add "HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\DataCollection" /v AllowTelemetry /t REG_DWORD /d 0 /f',
      description: 'Telemetria letilt├ísa (rendszab├íly)',
    });
    commands.push({
      ...
      description: 'Telemetria letilt├ísa (adatgy┼▒jt├ęs)',
    });
  }
  ...
  if (config.disableUAC) {
    commands.push({
      ...
      description: 'UAC (Felhaszn├íl├│i fi├│kok fel├╝gyelete) kikapcsol├ísa',
    });
  }

  // --- Gyorsind├şt├ís kikapcsol├ísa ---
  if (config.disableFastStartup) {
    commands.push({
      ...
      description: 'Gyorsind├şt├ís kikapcsol├ísa',
    });
  }

  // Alv├ís letilt├ísa ├ęs teljes├ştm├ęny energias├ęma kiker├╝lt innen, most a specialize f├ízisban van

  // Eg├ęrgyorsul├ís kikapcsol├ísa kiker├╝lt innen, most a specialize f├ízisban van (Default User)

  // --- 8. Egy├ęni Szkriptek (FirstLogon) ---
  if (config.customScripts) {
    ...
    if (config.customScripts.wingetApps === 'versionA') {
      ...
      scriptPaths.push(addBase64ScriptToFirstLogon(
        ...
        'Egy├ęni Szkript: Winget Appok telep├şt├ęse (A verzi├│)'
      ));
      scriptCounter++;
      ...
      scriptPaths.push(addBase64ScriptToFirstLogon(
        ...
        `C:\\Windows\\Temp\\custom_script_${scriptCounter}.b64`,
        `C:\\Windows\\Temp\\custom_script_${scriptCounter}.ps1`,
        'Egy├ęni Szkript: Winget Appok telep├şt├ęse (B verzi├│)'
      ));
      scriptCounter++;
...
function Wait-ForWinget {
  ...
  if (-not (Get-Command winget.exe -ErrorAction SilentlyContinue)) {
    Show-PopupAsync "Az Appok telep├şt├ęse sor├ín hiba l├ępett fel (Winget nem tal├ílhat├│)!" "Telep├şt├ęs"
    exit 1
  }
}
...
if($ok){
  Show-PopupAsync "Az Appok telep├şt├ęse sikeresen megt├Ârt├ęnt!" "Telep├şt├ęs"
}else{
  Show-PopupAsync "Az Appok telep├şt├ęse sor├ín hiba l├ępett fel!\`nR├ęszletek: C:\\InstallFull.log" "Telep├şt├ęs"
  throw "Winget telep├şt├ęsi hiba!"
}`;
      ...
      scriptPaths.push(addBase64ScriptToFirstLogon(
        ...
        `C:\\Windows\\Temp\\custom_script_${scriptCounter}.b64`,
        `C:\\Windows\\Temp\\custom_script_${scriptCounter}.ps1`,
        'Egy├ęni Szkript: Winget Appok telep├şt├ęse'
      ));
      scriptCounter++;
    }
    ...
    if (config.customScripts.office === 'versionA') {
      scriptPaths.push(addBase64ScriptToFirstLogon(
        ...
        `C:\\Windows\\Temp\\custom_script_${scriptCounter}.b64`,
        `C:\\Windows\\Temp\\custom_script_${scriptCounter}.ps1`,
        'Egy├ęni Szkript: Microsoft Office telep├şt├ęse (A verzi├│)'
      ));
      scriptCounter++;
      ...
      scriptPaths.push(addBase64ScriptToFirstLogon(
        ...
        `C:\\Windows\\Temp\\custom_script_${scriptCounter}.b64`,
        `C:\\Windows\\Temp\\custom_script_${scriptCounter}.ps1`,
        'Egy├ęni Szkript: Microsoft Office telep├şt├ęse (B verzi├│)'
      ));
      scriptCounter++;
    }
    ...
    if (config.customScripts.pcManager) {
      scriptPaths.push(addBase64ScriptToFirstLogon(
        ...
        `C:\\Windows\\Temp\\custom_script_${scriptCounter}.b64`,
        `C:\\Windows\\Temp\\custom_script_${scriptCounter}.ps1`,
        'Egy├ęni Szkript: PC Manager telep├şt├ęse'
      ));
      scriptCounter++;
    }
    ...
    if (config.customScripts.domainJoin) {
      scriptPaths.push(addBase64ScriptToFirstLogon(
        ...
        `C:\\Windows\\Temp\\custom_script_${scriptCounter}.b64`,
        `C:\\Windows\\Temp\\custom_script_${scriptCounter}.ps1`,
        'Egy├ęni Szkript: Active Directory Tartom├ínyba l├ęptet├ęs'
      ));
      scriptCounter++;
    }

    // Windows Update (Utols├│k├ęnt a takar├şt├ís el┼Ĺtt)
    if (config.customScripts.windowsUpdate) {
      scriptPaths.push(addBase64ScriptToFirstLogon(
        ...
        `C:\\Windows\\Temp\\custom_script_${scriptCounter}.b64`,
        `C:\\Windows\\Temp\\custom_script_${scriptCounter}.ps1`,
        'Egy├ęni Szkript: Windows Update interakt├şv keres├ęs'
      ));
      scriptCounter++;
    }
  }
...
Remove-Item -Path C:\\Windows\\Temp\\* -Recurse -Force -ErrorAction SilentlyContinue
  ...
  const cleanupPath = addBase64ScriptToFirstLogon(
    ...
    'V├ęgs┼Ĺ takar├şt├ís (Ideiglenes f├íjlok ├ęs szkriptek t├Ârl├ęse)'
  );
  scriptPaths.push(cleanupPath);

  // --- RunAll.ps1 Karmester (Orchestrator) gener├íl├ísa ---
  // Egyetlen PowerShell ablak ny├şlik meg, ami sorban megh├şvja az ├Âsszes el┼Ĺk├ęsz├ştett szkriptet.
  if (scriptPaths.length > 0) {
    ...
    for (let i = 0; i < runAllB64.length; i += runAllChunkSize) {
      ...
      commands.push({
        ...
        description: `Karmester szkript (adatok ├şr├ísa ${i + 1}. r├ęsz)`
      });
    }

    commands.push({
      command: `certutil.exe -decode -f ${runAllB64Path} ${runAllPs1Path}`,
      description: 'Karmester szkript (dek├│dol├ís)'
    });

    commands.push({
      command: `powershell.exe -WindowStyle Hidden -NoProfile -ExecutionPolicy Bypass -File ${runAllPs1Path}`,
      description: 'Karmester: ├ľsszes szkript futtat├ísa egyetlen ablakban'
    });
  }
