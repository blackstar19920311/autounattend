...
const customScriptsPath = 'D:\\OneDrive - Bányai Bajai SZC\\Projektek\\autounattend\\src\\utils\\customScripts.js';
let content = fs.readFileSync(customScriptsPath, 'utf8');

function escapeForTemplateLiteral(str) {
  return str
    .replace(/\\/g, '\\\\')
    .replace(/`/g, '\\`')
    .replace(/\\`\\n/g, '\\`\\n') // backticks at end of line (PowerShell line continuation) remain \`\n
    .replace(/\$/g, '\\$');
}

// 1. Rewrite officeA
let officeA = `\\$ErrorActionPreference = 'Continue'
...
let officeB = `\\$ErrorActionPreference = 'Continue'
\\$ProgressPreference = 'SilentlyContinue'
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
...
const prefix = content.substring(0, content.indexOf('  officeA: `'));
const suffix = content.substring(content.indexOf('  pcManager: `'));

const newContent = prefix + '  officeA: `' + officeA + '`,\n\n  officeB: `' + officeB + '`,\n\n' + suffix;

fs.writeFileSync(customScriptsPath, newContent);
