...
// The original file is untouched, wingetAppsA and B are perfect.
// Let's replace officeA and officeB using regex or string splitting.
...
const officeA = `  officeA: \`$ErrorActionPreference = 'Continue'
...
const officeB = `  officeB: \`$ErrorActionPreference = 'Continue'
$ProgressPreference = 'SilentlyContinue'
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
...
fs.writeFileSync('D:\\OneDrive - Bányai Bajai SZC\\Projektek\\autounattend\\src\\utils\\customScripts.js', prefix + officeA + officeB + suffix);
