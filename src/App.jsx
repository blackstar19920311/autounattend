...
export default function App() {
  ...
  const handleGenerate = useCallback(() => { const validation = validateConfig(config, t); if (!validation.isValid) { setErrors(validation.errors); showStatus('error', t('app.status.error.validation')); const el = document.getElementById(Object.keys(validation.errors)[0]); if (el) el.scrollIntoView({ behavior: 'smooth', block: 'center' }); return } try { setXml(applyGeneratorGuards(generateXml(config, config.installLanguage || language), config)); setErrors({}); showStatus('success', t('app.status.success')) } catch (err) { console.error('XML generation error:', err); showStatus('error', t('app.status.error.generation')) } }, [config, showStatus, t, language]);
