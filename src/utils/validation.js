...
export function validateConfig(config, t = key => key) {
  const errors = {}; const tr = (k, fallback) => t(k) === k ? fallback : t(k);
  const username = String(config.username || '').trim();
  if (!username) errors.username = tr('val.username.req', 'Username is required.');
  else if (username.length > 20 || /^[ .]|[ .]$|[\\/\[\]:|<>+=;,?*"]/.test(username) || RESERVED.test(username)) errors.username = 'Invalid or reserved Windows account name.';
  const name = String(config.computerName || '').trim();
  const max = config.randomSuffix === true ? 8 : 15;
  if (!name) errors.computerName = 'Computer name is required.';
  else if (name.length > max || /[^A-Za-z0-9-]/.test(name) || NAME_RESERVED.test(name)) errors.computerName = 'Invalid computer name or prefix.';
  ...
  if (wifi.mode === 'auto') {
    if (!wifi.ssid?.trim() || CONTROL.test(wifi.ssid) || new TextEncoder().encode(wifi.ssid).length > 32) errors['wifi.ssid'] = 'SSID must be 1-32 bytes and contain no control characters.';
    if (!wifi.password || CONTROL.test(wifi.password) || wifi.password.length > 63) errors['wifi.password'] = 'Wi-Fi password must be 8-63 characters and contain no control characters.';
  }
