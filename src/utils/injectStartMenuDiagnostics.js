...
function commandXml(order, command, description) {
  return ['        <SynchronousCommand wcm:action="add">', `          <Order>${order}</Order>`, `          <CommandLine>${escapeXml(command)}</CommandLine>`, `          <Description>${escapeXml(description)}</Description>`, '        </SynchronousCommand>'].join('\n');
}
...
function createDiagnosticCommands() {
  ...
  const commands = [];
  let order = 1;
  for (let i = 0; i < base64.length; i += 180) {
    commands.push(commandXml(order++, `cmd.exe /c ${i === 0 ? '>' : '>>'}${b64Path} echo ${base64.substring(i, i + 180)}`, 'Start menü diagnosztika telepítése'));
  }
...
export function injectStartMenuDiagnostics(xml) {
  ...
  const firstBlock = xml.slice(firstOpen, firstClose);
  const diagnostics = createDiagnosticCommands();
  const shifted = firstBlock.replace(/<Order>(\d+)<\/Order>/g, (_, value) => `<Order>${Number(value) + diagnostics.count}</Order>`);
  const withFirst = shifted.replace('<FirstLogonCommands>', `<FirstLogonCommands>\n${diagnostics.commands.join('\n')}`);
  let updated = xml.slice(0, firstOpen) + withFirst + xml.slice(firstClose);

  const lastIndex = updated.lastIndexOf(closeTag);
  const lastBlockStart = updated.lastIndexOf('<FirstLogonCommands>', lastIndex);
  const orders = [...updated.slice(lastBlockStart, lastIndex).matchAll(/<Order>(\d+)<\/Order>/g)].map(m => Number(m[1]));
  const lastOrder = (orders.length ? Math.max(...orders) : 0) + 1;
  const finalCommand = commandXml(lastOrder, `powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File ${diagnostics.scriptPath} -LogSuffix 2`, 'Start menü diagnosztika 2. mérés');
  return updated.slice(0, lastIndex) + finalCommand + '\n' + updated.slice(lastIndex);
}
